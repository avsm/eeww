...
(** Integer remainder. If [y] is not zero, the result of [rem x y] satisfies the
   following property: [x = add (mul (div x y) y) (rem x y)]. if [y = 0], [rem x
...
(** [shift_right_logical x y] shifts [x] to the right by [y] bits. This is a
   logical shift: zeroes are inserted in the vacated bits regardless of the sign
...
(** Convert the given {!t} integer to an integer (type [int]). On 64-bit
   platforms, the conversion is exact. On 32-bits platforms, the 32-bit integer
   is taken modulo 2{^31}, i.e. the high-order bit is lost during the
   conversion. *)
...
(** Convert the given string to a {!t} integer. The string is read in decimal
   ...
   The [0u] prefix reads the input as an unsigned integer in the range [[0, 2 *
   max_int + 1]]. If the input exceeds {!max_int} it is converted to the signed
   integer [min_int + input - max_int - 1].
   ...
   the integer represented exceeds the range of integers representable in type
   {!t}. *)
