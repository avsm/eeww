...
let task_events q =
  ...
  let id_callback d ts c i = match Runtime_events.User.tag c with
    ...
    | _ -> ()
  in
  let id_label_callback _d _ts c (i, s) = match Runtime_events.User.tag c with
...
module Task = struct
  ...
  let create ~id ~domain start = { id; domain; start; busy = 0L; entered_count = 0; info = []; active = false }
end
...
module Task_table = struct
  ...
  let remove_by_id tbl id = 
    filter (fun t -> match Lwd_table.get t with Some t -> Int.equal t.Task.id id | None -> true) (Lwd_table.first tbl)

  let update_loc tbl id info =
    map (fun t -> if Int.equal t.Task.id id then { t with info = info :: t.info } else t) (Lwd_table.first tbl)
  
  let update_active tbl id =
    map (fun t -> if Int.equal t.Task.id id then { t with active = true; entered_count = t.entered_count + 1 } else { t with active = false }) (Lwd_table.first tbl)
end
...
module Console = struct
  ...
  let render_task (prev, now) _ ({ Task.id; domain; start; info; active; busy; _ } as t) =
    let busy = 
    ...
    let idle = Int64.sub total busy in
  ...
  let ui_monoid_list : ui list list Lwd_utils.monoid = 
    ([], List.append)
  ...
  let root () =
    ...
    let widths = 
      Lwd.map ~f:(fun uis ->
        let widths = List.fold_left (fun acc w -> set_column_widths acc w) init_widths uis in
        widths
      ) task_list
    in
    let table = Lwd.map2 
      ~f:(fun widths ts ->
        ...
        List.fold_left (fun acc ui -> Ui.join_y (List.fold_left Ui.join_x Ui.empty (resize_uis widths ui)) acc) Ui.empty ts
      ) widths task_list 
    in
    let table_header = 
      Lwd.map widths ~f:(fun w -> List.map2 (fun w ui -> Ui.resize ~bg:Notty.A.(bg green) ~w:(w + padding) ~pad:gravity_pad ui) w header)
  ...
  let remove_task i =
    Task_table.remove_by_id tasks i

  let update_loc i id =
    Task_table.update_loc tasks i id

  let switch_to i =
    Task_table.update_active tasks i
end
  ...
  Nottui.Ui_loop.run ~tick:(fun () ->
    ...
    while not (Queue.is_empty q) do 
      ...
      | Some (`Resolved (_v, _, _)) -> 
        ()
        (* Console.remove_task v *)
      | Some (`Labelled (i, l)) -> Console.update_loc (i :> int) l
    done) ~tick_period:0.1 (Console.root ())
