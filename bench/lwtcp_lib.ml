...
let run_cp block_size queue_depth infile outfile () =
  Lwt_main.run
    ...
    let pool =
      Lwt_pool.create queue_depth (fun () ->
          Lwt.return @@ Bytes.make block_size '\000')
    in
    Logs.debug (fun l ->
        l "lwtcp: %s -> %s size %d queue %d bs %d" infile outfile insize
          queue_depth block_size);
    copy_file infd outfd insize block_size pool >>= fun () ->
    Logs.debug (fun l -> l "lwtcp: done");
    Lwt_unix.close outfd >>= fun () -> Lwt_unix.close infd)
